...
def page_parse(html):
    ...
    # 诗句链接
    sentence = soup.select('body > div.main3 > div.left > div.sons > div.cont > a:nth-of-type(1)')
    sentence_list = []
    href_list = []
    ...
    for i in range(len(info)):
        ...
        href_list.append("https://www.guwendao.net" + href)
...
def sub_page_parse(sub_html):
    ...
    for html in sub_html:
        soup = BeautifulSoup(html, 'lxml')
        poem = soup.select('div.main3 > div.left > div.sons > div.cont > div.contson')
        if len(poem) == 0:
            continue
...
if __name__ == '__main__':
    ...
    poemCount = 0
    for i in range(1, 5):
        url = 'https://www.guwendao.net/mingjus/default.aspx?page=%d' % i
        print(url)
        # time.sleep(1)
        html = page_request(url, ua)
